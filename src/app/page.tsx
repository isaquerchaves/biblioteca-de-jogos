...
function App() {
  return (
    <div className="flex flex-col justify-center items-center">
      <h1>Meus Jogos</h1>
      <hr className="flex-grow border-t border-gray-300 w-[75%]" />
      <div className="flex flex-wrap justify-around gap-16 items-center w-[75%]">
        {games.map((game) => {
          return (
            <div key={game.id} className=" max-w-[20rem]">
                <img src={game.coverImage} alt={game.title} className="h-[30rem] w-[20rem]" />
                <h2>{game.title}</h2>
